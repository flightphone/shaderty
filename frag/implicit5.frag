...
vec3 knot(float t) {
    return vec3(sin(t) + 2. * sin(2. * t) - 0.1, cos(t) - 2. * cos(2. * t), sin(3. * t) - 0.1);
}
...
float digdong(vec3 p) {
    //p = clamp(p, vec3(-1.), vec3(1.));
    float k = 3., x = p.x * k, y = p.y * k, z = p.z * k, a = 2.;
    float t = (x * x + y * y) * 2.5 - (a - z) * z * z;
    float f = .5;
    t += pow(min(f+z, 0.), 4.);
    return t;
}
...
float chair(vec3 p) {
    ...
    float s = 0.6, x = p.x * s, y = p.y * s, z = p.z * s, a = 0.8, k = 1.2, b = 0.4, d = x * x + y * y + z * z - a * k * k;
...
float gr(vec2 p) {
    float r = 1.5;
    if(length(p) < r) {
        float x1 = (p.x + r) / 2.0 / r, y1 = (p.y + r) / 2.0 / r;
...
float noiseSph(vec3 p)
{
    ...
    //vec2 ll = vec2(4.*sin(lon), 2.*cos(lat));
    //float ll = sin(4.*lon) +  cos(7.*lat) + iTime*0.3;
    //float ll = sin(3.*lon) +  cos(5.*lat) + iTime*0.3;
    //float r = 1. +  0.1*noise(p*3.) + noise(p);// 
    //float r = 2. + 0.5 * noise(ll)+  0.1*noise(ll*5.)  ;
    //float r = 2. + 0.5 * noise(ll)+  0.2*noise(ll*5.) + 0.05*noise(ll*9.)  ;
    //float r = 1. + 0.7 * noise(p+iTime/3.) +  0.2*noise((p+iTime/2.)*5.);
