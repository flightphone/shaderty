...
float pluspole3(vec3 p, float r) {
    //p*=scale;
...
float sch(vec3 p) {
    float scale = 2. * PI;
    p *= scale;
    float r = cos(p.x) + cos(p.y) + cos(p.z);
    r = r * r - 0.1 + pluspole3(p, 2. * PI);
    return r;
}
...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
    ...
    for(int m = 0; m < AA; m++) for(int n = 0; n < AA; n++) {
            ...
            vec2 d2 = boxIntersection(ro, rd, vec3(dist));
            if(d2[0] < dist_inf) {
