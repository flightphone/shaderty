...
HIT giper3D(vec3 ro, vec3 rd, float t, float r)
{
    ...
    float a0 = 1.*a*a*e + 1.*c*c*e-1.*e*t*t-1.*a*a*t + 1.*c*c*t;
    float a1 = 1.*a*a*f + 1.*c*c*f-1.*f*t*t + 2.*a*b*e + 2.*c*d*e-2.*a*b*t + 2.*c*d*t;
    float a2 = 2.*a*b*f + 2.*c*d*f + 1.*b*b*e + 1.*d*d*e-1.*b*b*t + 1.*d*d*t;
    ...
    */
    ...
    if (dist < dist_infin)
    {
        nor = vec3(0.+2.*pos.x*pos.z-2.*t*pos.x, 0.+2.*pos.y*pos.z+2.*t*pos.y, 0.+1.*pos.x*pos.x+1.*pos.y*pos.y-1.*t*t);
    ...
    for( int n=0; n<AA; n++ )
    {
        ...
        if (giper.dist < dist)
        {
           col = vec3(0.5, 0.5, 1.0);
            ...
            //backcol = pow(backcol, vec3(0.4545));
