...
HIT giper3D(vec3 ro, vec3 rd, float t, float r)
{
    ...
    float a0 = 1.*a*a*e*e + 1.*c*c*e*e-1.*t*t*t*t;
    float a1 = 2.*a*a*e*f + 2.*c*c*e*f + 2.*a*b*e*e + 2.*c*d*e*e;
    float a2 = 1.*a*a*f*f + 1.*c*c*f*f + 4.*a*b*e*f + 4.*c*d*e*f + 1.*b*b*e*e + 1.*d*d*e*e;
    float a3 = 2.*a*b*f*f + 2.*c*d*f*f + 2.*b*b*e*f + 2.*d*d*e*f;
    float a4 = 1.*b*b*f*f + 1.*d*d*f*f;
    ...
    if (dist < dist_infin)
    {
        nor = vec3(0.+2.*pos.x*pos.z*pos.z, 0.+2.*pos.y*pos.z*pos.z, 0.+2.*pos.x*pos.x*pos.z+2.*pos.y*pos.y*pos.z);
        nor = normalize(nor);
    }
